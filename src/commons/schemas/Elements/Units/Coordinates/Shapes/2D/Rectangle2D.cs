...
namespace Mov.Schemas.Coordinates.Shapes.Layers
{
    public sealed class Rectangle2D : ValueObjectBase<Rectangle2D>
    {
        ...
        public Rectangle2D(Point2D upperLeft, Point2D lowerRight)
        {
            ...
            this.CenterPoint = Point2D.Factory.New(new CoordinateUnit((lowerRight.X.Value + upperLeft.X.Value) / 2), new CoordinateUnit((lowerRight.Y.Value + upperLeft.Y.Value) / 2));
        ...
        public Rectangle2D(Point2D center, LengthUnit width, LengthUnit height)
        {
            ...
            this.Width = width;
            this.Height = height;
            this.UpperLeftPoint = Point2D.Factory.New(new CoordinateUnit(center.X.Value - (width.Value / 2)), new CoordinateUnit(center.Y.Value + (height.Value / 2)));
            this.LowerRightPoint = Point2D.Factory.New(new CoordinateUnit(center.X.Value + (width.Value / 2)), new CoordinateUnit(center.Y.Value - (height.Value / 2)));
        }
