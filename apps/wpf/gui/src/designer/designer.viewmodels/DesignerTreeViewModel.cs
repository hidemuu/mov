...
namespace Mov.Designer.ViewModels
{
    public class DesignerTreeViewModel : DragDropViewModelBase
    {
        ...
        private void GetLayoutTrees(ICollection<NodeSchema> items, IEnumerable<DesignerTreeModel> models)
        {
            foreach (var model in models)
            {
                var item = new NodeSchema
                {
                    Id = model.Id.Value,
        ...
        private void GetContentTables(ICollection<ContentSchema> items, IEnumerable<DesignerTreeModel> models)
        {
            foreach (var model in models)
            {
                var item = new ContentSchema
                {
                    Id = model.Id.Value,
                    Index = model.Index.Value,
                    Code = model.Code.Value,
    ...
    public class DesignerTreeModel : DesignerTableModel
    {
        ...
        protected override void Update(ContentSchema item)
        {
            if (item == null) return;
            var table = new ContentSchema(item);
            table.Id = Id.Value;
